...
class APICallManager {
    ...
    func callAPIGetOffers(offset:String, onSuccess successCallback: ((_ user: OffersModel) -> Void)?, onFailure failureCallback: ((_ errorMessage: String) -> Void)?) {
        
        
        // Build URL
        let url = API_BASE_URL + Endpoint.Offers.rawValue + "?offset=0"
        ...
        self.createRequest(
            ...
            onSuccess: {(responseObject: JSON) -> Void in
                ...
                if let responseDict = responseObject.dictionaryObject {
                    let loginDict = responseDict as [String : AnyObject]
                    let data =  OffersModel.build(loginDict)
